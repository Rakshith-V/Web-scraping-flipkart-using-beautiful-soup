...
query="select urls,status from oven_details where status='0'"
...
for row in a:
    ...
    cur.execute("SHOW COLUMNS FROM oven_details")
    b=cur.fetchall()
    for row in b:
    ...
    if( status == 0):
        ...
        for i,j in zip(column,row):
            ...
            if column in column_name:
                ...
                cur.execute("ALTER table oven_details ADD %s text"%(column))
                db.commit()
            cur.execute("UPDATE oven_details SET %s='%s' where urls='%s' "%(column,values,link))
            db.commit()
            print(column,"================>",values)
            cur.execute("UPDATE oven_details SET status='1' where urls='%s' "%(link))
            db.commit()
