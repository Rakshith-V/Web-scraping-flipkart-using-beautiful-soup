...
query="select urls,status from laptop_details limit 223,800"
#executing query with the cursor
cur.execute(query)
a=cur.fetchall()
passvar = 1
#creating price column once
#cur.execute("ALTER table laptop_details ADD price text")
#db.commit()

for row in a:
    print("LAPTOP",passvar,"\n")
    passvar=passvar+1
    ...
    cur.execute("UPDATE laptop_details SET price='%s' where urls='%s' "%(price,link))
    db.commit()
    ...
    '''for a in link2:
        ...
        if column in column_name:
            ...
            db.commit()
        cur.execute("UPDATE laptop_details SET %s='%s' where urls='%s' "%(column,src,link))
        db.commit()'''
    print("\n")
db.close()            
