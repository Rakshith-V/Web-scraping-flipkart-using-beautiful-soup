...
url = "https://www.flipkart.com/search?q=refrigerator&sid=j9e%2Cabm%2Chzg&as=on&as-show=on&otracker=AS_QueryStore_HistoryAutoSuggest_0_4&otracker1=AS_QueryStore_HistoryAutoSuggest_0_4&as-pos=0&as-type=HISTORY&as-backfill=on&page="
...
while x <= 25:
    ...
    for a in links:
        ...
        query= "insert into refrigerator_details(urls) values('%s')"%(all_links)
        print(query)
        cur.execute(query)
        db.commit()
    x =x+1
