...
query="select urls,status from refrigerator_details where status='0'"
...
for row in a:
    ...
    cur.execute("SHOW COLUMNS FROM refrigerator_details")
    b=cur.fetchall()
    for row in b:
    ...
    if( status == 0):
        ...
        for i,j in zip(column,row):
            ...
            column = i.text
            values = j.text
             #sanitizing 'column' and 'value' to remove un-necessary spaces and special characters  
            column = column.strip()
            ...
            if column in column_name:
                ...
                cur.execute("ALTER table refrigerator_details ADD %s text"%(column))
                db.commit()
            cur.execute("UPDATE refrigerator_details SET %s='%s' where urls='%s' "%(column,values,link))
            db.commit()
            print(column,"================>",values)
            cur.execute("UPDATE refrigerator_details SET status='1' where urls='%s' "%(link))
            db.commit()
