...
url = "https://www.flipkart.com/search?q=air+conditioners&as=on&as-show=on&otracker=AS_Query_HistoryAutoSuggest_0_6&otracker1=AS_Query_HistoryAutoSuggest_0_6&as-pos=0&as-type=HISTORY&as-backfill=on&page="
...
while x <= 17:
    ...
    for a in links:
        ...
        query= "insert into ac_details(urls) values('%s')"%(all_links)
        print(query,"\n")
        cur.execute(query)
        db.commit()
    x =x+1
