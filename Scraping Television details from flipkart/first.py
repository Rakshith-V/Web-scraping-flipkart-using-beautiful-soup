...
url = "https://www.flipkart.com/search?q=TV&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off&page="
...
while x <= 60:
    ...
    for a in links:
        ...
        query= "insert into tv_details(urls) values('%s')"%(all_links)
        print(query)
        cur.execute(query)
        db.commit()
    x =x+1
