...
query="select urls,status from mobiles_details limit 460,1000"
#executing query with the cursor
cur.execute(query)
a=cur.fetchall()
passvar = 1
#creating price column once
#cur.execute("ALTER table mobiles_details ADD price text")
#db.commit()
...
for row in a:
    ...
    # cur.execute("UPDATE mobiles_details SET price='%s' where urls='%s' "%(price,link))
    # db.commit()
    ...
    #         db.commit()
    #     cur.execute("UPDATE mobiles_details SET %s='%s' where urls='%s' "%(column,src,link))
    #     db.commit()
    # print("\n")
db.close()            
