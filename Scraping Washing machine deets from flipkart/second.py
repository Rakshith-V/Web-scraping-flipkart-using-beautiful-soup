...
query="select urls,status from washingmachine_details where status='0'"
    ...
    if( status == 0):
        ...
        for i,j in zip(column,row):
            ...
            if column in column_name:
                ...
                db.commit()
            cur.execute("UPDATE washingmachine_details SET %s='%s' where urls='%s' "%(column,values,link))
            db.commit()
            print(column,"================>",values)
            cur.execute("UPDATE washingmachine_details SET status='1' where urls='%s' "%(link))
            db.commit()
