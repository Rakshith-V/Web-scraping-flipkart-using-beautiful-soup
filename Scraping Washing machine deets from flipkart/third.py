...
query="select urls,status from washingmachine_details"
#executing query with the cursor
cur.execute(query)
a=cur.fetchall()
passvar = 1
#creating price column once
#cur.execute("ALTER table washingmachine_details ADD price text")
#db.commit()

for row in a:
    print("Washing machhine",passvar,"\n")
    passvar=passvar+1
    ...
    cur.execute("SHOW COLUMNS FROM washingmachine_details")
    b=cur.fetchall()
    for row in b:
    ...
    cur.execute("UPDATE washingmachine_details SET price='%s' where urls='%s' "%(price,link))
    db.commit()
    ...
    '''for a in link2:
        ...
        if column in column_name:
            ...
            cur.execute("ALTER table washingmachine_details ADD %s text"%(column))
            db.commit()
        cur.execute("UPDATE washingmachine_details SET %s='%s' where urls='%s' "%(column,src,link))
        db.commit()'''
    print("\n")
db.close()            
